...
export default function PharmacistRegistration() {
  ...
  const [registeredPharmacyId, setRegisteredPharmacyId] = useState('');
  ...
  const getCurrentLocation = () => new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (geoError) => {
        ...
        reject(new Error('Error getting location. Please try again.'));
      }
    );
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (resp?.pharmacy?.pharmacyId) {
        setRegisteredPharmacyId(resp.pharmacy.pharmacyId);
        ...
        <div className="form-card">
          ...
          >
            ...
            <input
              ...
              onChange={handleChange}
            />
            {locationError && <div className="register-error location-error">⚠️ {locationError}</div>}
            ...
            <button type="submit" className="register-button" disabled={submitting || locationLoading}>
              {locationLoading ? 'Capturing Location...' : submitting ? 'Registering...' : 'Register'}
            </button>
          </form>
        </div>
      </div>
      ...
      {showSuccessModal && (
        <div className="id-modal-overlay" onClick={() => setShowSuccessModal(false)}>
          <div className="id-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p>Your Pharmacy ID is:</p>
            <div className="id-code">{registeredPharmacyId}</div>
            <p>Use this ID or your email to login.</p>
            <button
              type="button"
