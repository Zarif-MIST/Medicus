...
export default function DiagnosisPage() {
  ...
  const addMedicine = () => {
    ...
    const normalizedQuantity = Math.max(1, parseInt(medForm.quantity, 10) || 1);
    const normalizedDurationDays = Math.max(1, parseInt(medForm.duration, 10) || 1);
    ...
    setMedicines((prev) => [
      ...prev,
      {
        ...
        duration: String(normalizedDurationDays),
      },
    ]);
    ...
    try {
      ...
      const response = await apiService.createPrescription({
        ...
        medicines: medicines.map(m => ({
          ...
          duration: `${Math.max(1, parseInt(m.duration, 10) || 1)} days`,
      ...
      if (response.prescription) {
        ...
        setCreatedPrescriptionId(response.prescription.prescriptionId || "");
  ...
  return (
    <div className="Diagnosis-page">
                ...
                <div key={m.id} className="med-item">
                  <div className="med-item-left">
                    ...
                    <p className="med-meta">
                        Quantity: {m.quantity} | Frequency: {m.frequency} | Duration: {m.duration} day(s)
                    </p>
                  </div>
                  <button
                    type="button"
          ...
          {!showMedForm ? (
            ...
            <>
              ...
              <p className="form-helper-text">
                💡 Duration starts from today and must be entered in days only.
              </p>
      ...
      {showSuccessModal && (
        <div className="id-modal-overlay" onClick={() => setShowSuccessModal(false)}>
          <div className="id-modal" onClick={(e) => e.stopPropagation()}>
            <h3>Prescription Sent Successfully</h3>
            <p>Prescription ID:</p>
            <div className="id-code">{createdPrescriptionId}</div>
            <p>The prescription has been sent to pharmacists.</p>
            <button
              type="button"
              className="send-btn"
              onClick={() => {
