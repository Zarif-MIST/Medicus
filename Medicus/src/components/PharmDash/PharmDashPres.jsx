...
export default function PharmDashPres() {
  ...
  const handleSearchPatient = async () => {
    ...
    try {
      setSearchError('');
      setPrescriptions([]);
      const response = await apiService.getPrescriptionsByPatient(patientSearchId, { forPharmacy: true });
  ...
  const handleDispenseMedicine = async () => {
    if (!calculatedQuantity || calculatedQuantity <= 0) {
      setDispensingError('Invalid prescribed quantity');
      return;
    }
    ...
    setDispensingError('');
    try {
      const latestRxResponse = await apiService.getPrescriptionById(selectedRx.prescriptionId);
      const latestRx = latestRxResponse?.prescription;

      if (latestRx) {
        setSelectedRx(latestRx);
        setPrescriptions((prev) =>
          prev.map((rx) =>
            rx.prescriptionId === latestRx.prescriptionId
              ? latestRx
              : rx
          )
        );

        const latestMedicine = latestRx.medicines?.[dispensingMedicine.medicineIndex];
        if (latestMedicine?.dispensed) {
          setDispensingError(`${latestMedicine.medicineName || dispensingMedicine.medicineName} is already dispensed.`);
      ...
      // Then mark this specific medicine as dispensed globally across pharmacies
      let dispenseResult;
      try {
        dispenseResult = await apiService.dispensePrescriptionMedicine(
          selectedRx.prescriptionId,
          dispensingMedicine.medicineIndex,
          user?.id
        );
      } catch (dispenseError) {
        // rollback inventory on failure to mark dispensed
        await apiService.updateMedicineQuantity(medicineInStock._id, quantityNeeded, 'restock');

        if ((dispenseError.message || '').toLowerCase().includes('already dispensed')) {
          const refreshedResponse = await apiService.getPrescriptionById(selectedRx.prescriptionId);
          const refreshedPrescription = refreshedResponse?.prescription;
          if (refreshedPrescription) {
            setSelectedRx(refreshedPrescription);
            setPrescriptions((prev) =>
              prev.map((rx) =>
                rx.prescriptionId === refreshedPrescription.prescriptionId
                  ? refreshedPrescription
                  : rx
              )
            );
          }
          setDispensingError(`${dispensingMedicine.medicineName} is already dispensed.`);
          setDispensingMedicine(null);
          setCalculatedQuantity(0);
          return;
        }
      ...
      // Refresh selected prescription and list from server response
      if (dispenseResult?.prescription) {
        setSelectedRx(dispenseResult.prescription);
        setPrescriptions((prev) =>
          prev.map((rx) =>
            rx.prescriptionId === dispenseResult.prescription.prescriptionId
              ? dispenseResult.prescription
              : rx
          )
        );
      }

      if (dispenseResult?.allMedicinesDispensed) {
        alert(`All medicines dispensed for prescription ${selectedRx.prescriptionId}.`);
      } else {
  ...
  const getPrescribedQuantity = (medicine) => {
    return Math.max(1, parseInt(medicine?.quantity, 10) || 1);
  };
          ...
          <div className="modal-content wide" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="summary-grid">
                ...
                <div className="pill-card prescription-id-pill">Pres ID<span>{selectedRx.prescriptionId}</span></div>
                ...
                <div className="medicines-list">
                  {selectedRx.medicines && selectedRx.medicines.length > 0 ? (
                    selectedRx.medicines.map((med, idx) => {
                      const isDispensed = !!med.dispensed;
                      ...
                      return (
                        <div key={idx} className={`medicine-card ${isDispensed ? 'dispensed' : ''}`}>
                          ...
                          <button 
                            ...
                            onClick={() => {
                              setDispensingMedicine({ ...med, medicineIndex: idx });
                              const qty = getPrescribedQuantity(med);
        ...
        <div className="modal-overlay" onClick={() => setDispensingMedicine(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="field">
                <label>Quantity to Dispense (From Prescription)</label>
                <input
                  type="number"
                ...
                <small style={{ color: '#666', fontSize: '13px', marginTop: '4px', display: 'block' }}>
                  Prescribed quantity: {calculatedQuantity}
                </small>
              </div>
